...
- (void)drawRect:(CGRect)rect {
    [super drawRect:rect];
    if (lvfjshowTimeDetail) [self drawDashLine:rect];
}
...
- (void)drawTimeLineMask:(CGContextRef)ctx rect:(CGRect)rect {
    ...
    NSDictionary *attribute = @{NSFontAttributeName:font,NSForegroundColorAttributeName:[UIColor YYStock_textColor]};
    ...
    attribute = @{NSFontAttributeName:font,NSForegroundColorAttributeName:[UIColor YYStock_textColor]};
    
    CGFloat gap = 95,x=75;
    [self.drawTimeLineDescTexts enumerateObjectsUsingBlock:^(NSString *  _Nonnull drawText, NSUInteger idx, BOOL * _Nonnull stop) {
        CGRect textRect = [self rectOfNSString:drawText attribute:attribute];
        CGRect drawRect = CGRectMake(x + gap * idx , (rect.size.height - textRect.size.height)/2.f, textRect.size.width, textRect.size.height);
        [drawText drawInRect:drawRect withAttributes:attribute];
    }];
    
    attribute = @{NSFontAttributeName:font,NSForegroundColorAttributeName:[UIColor YYStock_topBarSelectedTextColor]};
    [[self drawTimeLineTexts] enumerateObjectsUsingBlock:^(NSString *  _Nonnull drawText, NSUInteger idx, BOOL * _Nonnull stop) {
        CGRect textRect = [self rectOfNSString:drawText attribute:attribute];
        CGRect drawRect = CGRectMake(x + 37 + gap * idx , (rect.size.height - textRect.size.height)/2.f, textRect.size.width, textRect.size.height);
        [drawText drawInRect:drawRect withAttributes:attribute];
    }];
    ...
    drawRect = CGRectMake(x + 290 , (rect.size.height - textRect.size.height)/2.f, textRect.size.width, textRect.size.height);
    [drawText drawInRect:drawRect withAttributes:attribute];
    
    
    attribute = @{NSFontAttributeName:font,NSForegroundColorAttributeName:[UIColor YYStock_topBarSelectedTextColor]};
    NSArray *array = [lvfjBusiness changeChineseNumber:[self.selectedModel Volume]];
    drawText = [NSString stringWithFormat:@"%@  %@",[array firstObject],[array lastObject]];
    textRect = [self rectOfNSString:drawText attribute:attribute];
    drawRect = CGRectMake(x + 340 , (rect.size.height - textRect.size.height)/2.f, textRect.size.width, textRect.size.height);
    [drawText drawInRect:drawRect withAttributes:attribute];
}
